...
const SideBar = ({ sidebarCollapsed, setSidebarCollapsed }) => {
	...
	const menuItems = [
		...
		{
			label: "Invest",
			icon: <RiMoneyDollarCircleLine size={!isMobileView ? "25" : "65"} />,
			path: "investnow",
			tab: "investnow",
									...
									<div key={item.tab}>
										...
										>
											<Link to={item.path}>
												...
												{!sidebarCollapsed && (
													<span
														className={
															currentTab === item.tab ? "items-active" : ""
														}
													>
														{item.label}
													</span>
										...
										{expandedPitch && item.tab === "pitch" && (
											<div className="sub-menu">
												<MenuItem
													active={
														currentTab === "pitchRecordings" ||
														(sidebarCollapsed && currentTab === "pitch")
													}
													onClick={() => {
