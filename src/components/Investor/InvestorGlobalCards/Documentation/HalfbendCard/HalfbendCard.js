...
const HalfbendCard = ({ folderName, userId }) => {
	...
	const renderFileIcon = (fileType, fileUrl) => {
		switch (fileType) {
			case "pdf":
				return <BiSolidFilePdf size={100} className="pdf-icon" />;
			case "image":
				return <img src={fileUrl} alt="Preview" className="file-preview" />;
			default:
				return <FaFileAlt size={100} className="file-icon" />;
		}
	};
		...
		<div className="half_bend_container row rowWidth">
			<div className="box_container mt-4">
				<div className="row rowWidth">
					{loading && (
						<SpinnerBS className="d-flex py-5 justify-content-center align-items-center w-100" />
					)}
					...
					{!loading &&
						data?.map((item) => {
							...
							let class_name, custom_card_class_name;
							if (item.videoUrl) {
							...
							return (
								<div
									className={`col-md-4 d-flex justify-content-center align-items-center main_col ${
										folderName === "onelinkpitch" ? "video_card" : "pdf_card"
									}`}
									key={item.fileName}
									style={{ width: "100%", padding: "1rem" }}
								>
									<div
										className={custom_card_class_name}
										style={{ width: "100%" }}
									>
										{item.videoUrl ? (
