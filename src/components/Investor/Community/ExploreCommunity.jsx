...
export default function ExploreCommunities() {
	...
	// 	if (!community.isOpen) {
			// toast(
			// 	<CustomToast message="This community is closed for new members" />,
			// 	{
			// 		position: "top-center",
			// 		autoClose: 3000,
	...
	// 	// Add validation that if loggedInUserId is already a member of the community
	// 	if (
	// 		community.members.some((member) => member.member._id === loggedInUserId)
	// 	) {
	// 		toast(
	// 			<CustomToast message="You are already a member of this community" />,
	// 			{
	// 				position: "top-center",
	// 				autoClose: 3000,
	// 				hideProgressBar: true,
	...
	const handleJoinCommunity = async (e, community) => {
		...
		const showToast = (message, isError = false) => {
			toast(<CustomToast message={message} />, {
				...
				pauseOnHover: true,
				draggable: false,
				className: `custom-toast-container ${isError ? "error" : ""}`,
			});
		};

		if (!community.isOpen) {
			showToast("This community is closed for new members");
			return;
		}
		...
		try {
			...
			// Ensure loggedInUser Id is valid
			if (!loggedInUserId) {
				showToast("User  ID is not valid", true);
				return;
			}
			...
			console.error("Error joining community:", error);
			const errorMessage =
				error.response?.data?.message || "Failed to join the community";
						...
						>
							...
							<div className="community-info">
								...
								{!community.members.some(
									(member) => member?.member?.toString() === loggedInUserId
								) &&
									community.isOpen &&
									community.adminId !== loggedInUserId && (
										<button
										...
										>
											{joiningCommunityId === community._id
												? "Joining..."
												: "Join"}
										</button>
									)}
