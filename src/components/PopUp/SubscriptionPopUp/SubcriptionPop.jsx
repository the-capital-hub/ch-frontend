...
const SubcriptionPop = ({ popPayOpen, setPopPayOpen }) => {
      ...
      if (item.subscriptionType === "Basic") {
        if (loggedInUser.trialStartDate) {
          ...
          if (res.message === "Trial started") {
            ...
            dispatch({ type: "user/updateTrialStartDate", payload: currentDate });
            setPopPayOpen(false);
  ...
  return (
    <>
      {popPayOpen && (
        <div className="subscriber-background-overlay" onClick={() => setPopPayOpen(false)}></div>
      )}
      <div className={`subscriber_post_modal rounded-4 p-md-2 ${popPayOpen ? "d-block" : ""}`} tabIndex="-1" role="dialog">
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="pricing_plans">
                {subscription.filter(item => item.subscriptionType !== "Standard").map((item, index) => (
                  <div className="plan" key={index} style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", border: item.subscriptionType === "Pro" ? "2px solid gold" : "none" }}>
                    <div>
                      <h3 className="headingContent" style={{ color: getColorBySubscriptionType(item.subscriptionType) }}>
                        {item.subscriptionType}
                        {item.subscriptionType !== "Basic" && (
                          <IoSparkles style={{ marginLeft: "10px", color: getColorBySubscriptionType(item.subscriptionType) }} />
                        )}
                      </h3>
                      <p className="price">
                        ₹{item.price} <span>/year</span>
                      </p>
                      <p className="description" style={{ border: "none", marginBottom: 0, fontSize: "12px" }}>
                        {item.description}
                      </p>
                    </div>
                    <hr />
                    <div className="features" style={{ height: "70%" }}>
                      {item.features.map((i, j) => (
                        <p key={j} style={{ marginBottom: 0, fontSize: "13px" }}>
                          <CiCircleCheck color={getColorBySubscriptionType(item.subscriptionType)} /> {i}
                        </p>
                      ))}
                    </div>
                    <button className="purchase_button" onClick={() => handlePurchase(item)}>
