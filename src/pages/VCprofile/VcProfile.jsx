...
const VcProfile = () => {
    ...
    useEffect(() => {
        const fetchVcData = async () => {
            try {
                const response = await fetch('https://api.thecapitalhub.in/vc/getVcById', {
                    method: 'POST',
                    headers: {
    ...
    return (
        ...
        <div className="vc-profile">
            ...
            <div className="vc-social-links">
                ...
                {vcData.linkedin && (
                    <a href={vcData.linkedin} className='linkedIn' target="_blank" rel="noopener noreferrer">
                        <FaLinkedinIn />
                    </a>
                )}
                <br/>
            ...
            <div className='vc-funding'>
                <a href={"/funding"} className="fundingApply" target="_blank" rel="noopener noreferrer">
                    Apply for Funding
                </a>
            </div>
            ...
            <div className="vc-contact-section">
                    ...
                    <h2 className="vc-contact-header">Stage Focus</h2>
                        <div className="vc-bubbles">
                            {vcData.stage_focus.map((stage, index) => (
                                <div key={index} className="bubble purple">{stage}</div>
                            ))}
                        </div>
                    </div>
                    <div className="vc-sector-focus">
                        ...
                        <div className="vc-bubbles">
                            {vcData.sector_focus.map((sector, index) => (
                                <div key={index} className="bubble purple">{sector}</div>
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="vc-details">
                    <div className="vc-detail-item">
                        <h4>Ticket Size:</h4>
                        <div className="vc-detail bubble grey">{vcData.ticket_size}</div>
                    </div>
                    <div className="vc-detail-item">
                        <h4>Total Portfolio:</h4>
                        <div className="vc-detail bubble grey">{vcData.total_portfolio}</div>
                    </div>
                    <div className="vc-detail-item">
                        <h4>Current Fund Corpus:</h4>
                        <div className="vc-detail bubble grey">{vcData.current_fund_corpus}</div>
                    </div>
                    <div className="vc-detail-item">
                        <h4>Total Fund Corpus:</h4>
                        <div className="vc-detail bubble grey">{vcData.total_fund_corpus}</div>
                    </div>
                </div>
            </div>
        ...
        <button onClick={handleBackClick} className="back-button">Back to VCs</button>
        </>
    );
};
