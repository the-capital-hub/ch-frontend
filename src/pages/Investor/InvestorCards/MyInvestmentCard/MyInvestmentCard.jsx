...
const MyInvestmentCard = ({
  ...
  return (
    <div className="investment-card-container rounded-4 position-relative d-flex flex-column" style={{backgroundColor: theme==="dark"?"#18181899":"#b1b0b04d"}}>
      <div className="d-flex flex-column py-2 px-3 border-bottom flex-grow-1" >
        <div className="left">
          <img src={getCompanyLogo()} alt="Logo" className="logo" />
          {editMode ? (
            <>
              <input
                type="text"
              ...
              <div className="mt-2">
                ...
                >
                  <BsFillCloudUploadFill style={{ fontSize: "1.5rem", color: "rgba(140, 90, 201, 1)" }} />
                </label>
                <input
                  type="file"
                  name="logoFile"
        ...
        {editMode ? (
          ...
          <p className={`m-0 mt-1 py-2 ${editMode ? "d-none" : "d-block"}`} style={{ maxHeight: "100px", minHeight: "100px", overflowY: "auto", fontSize: "14px", 
            scrollbarWidth: "none", /* For Firefox */
            msOverflowStyle: "none", /* For Internet Explorer and Edge */
            }}>
            <span style={{color: theme==="dark"?"#DDFF71":"#7a8e38"}}><b>Company description:</b></span> {getCompanyDescription()}
          </p>
        )}
      </div>
      {!isInterests ? (
        <div className="bottom d-flex align-items-center py-2 px-3 gap-2" style={{backgroundColor:theme==="dark"?"":"#bcbcbc99", borderRadius:"10px"}}>
          <img src={InvestedIcon} alt="" className="small-image" />
          <p className="m-0 text-secondary">
            Invested:{" "}
            <span className={`equity d-l-grey  ms-2 ${editMode ? "d-none" : ""}`}>
        ...
        <>
          <div className="d-flex align-items-center py-2 px-3" style={{backgroundColor:theme==="dark"?"":"#bcbcbc99", borderRadius:"10px"}}>
            <p className={`m-0`}>
            ...
            {editMode && (
              <div className="dropdown flex-grow-1 ms-2">
                ...
                <ul className="dropdown-menu m-0 p-0 w-100">
                  {["N.A", "Soft commitment", "Due diligence phase"].map((option, idx) => (
      ...
      {editMode && (
        ...
        >
          {loading ? <SpinnerBS colorClass={"text-dark"} spinnerSizeClass="spinner-border-sm" /> : "Save"}
        </button>
      )}
      <button
